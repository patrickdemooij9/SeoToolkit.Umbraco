...
**Checksum verification (spec §4 open item):** `DeployArtifactBase` computes its checksum lazily on first read. Confirm in `~/Source/github/umbraco/Umbraco-Deploy/src/Umbraco.Deploy.Infrastructure/Artifacts/DeployArtifactBase.cs` + `ArtifactBase.cs` whether (a) the checksum has already been read by the time `ArtifactExportedNotification` fires, and (b) whether `Dependencies` participates in the checksum. If dependency mutation after checksum computation causes signature mismatches during transfers, switch the handler to `ArtifactExportingNotification` (fires before export completes) — same handler body, different notification type. Record the outcome in the plan file when executing.
