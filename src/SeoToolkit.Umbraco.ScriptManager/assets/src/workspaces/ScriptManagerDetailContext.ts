...
export default class ScriptManagerDetailContext
  ...
  implements UmbWorkspaceContext, UmbRoutableWorkspaceContext
{
  ...
  #definitions = new UmbArrayState<ScriptDefinitionViewModel>(
    [],
    (item) => item.alias
  );
  ...
  async save() {
    ...
    if (isNew) {
      const domainId = new URLSearchParams(window.location.search).get(
        "domainId"
      );
      if (domainId) {
    ...
    const response = await this.#repository.saveScript(script);
    if (response.error) {
      return;
    }
...
export const ST_SCRIPTMANAGER_DETAIL_TOKEN_CONTEXT =
  new UmbContextToken<ScriptManagerDetailContext>("scriptManagerDetailContext");
