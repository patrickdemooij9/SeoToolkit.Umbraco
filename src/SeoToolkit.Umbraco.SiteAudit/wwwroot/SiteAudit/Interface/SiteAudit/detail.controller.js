angular.module("umbraco").component("siteAuditDetail", {
    templateUrl: "/App_Plugins/SeoToolkit/Interface/SiteAudit/detail.html",
    controller: ["$routeParams", "$http", "eventsService", "siteAuditHub", function ($routeParams, $http, eventsService, siteAuditHub) {
        this.$onInit = function () {
            ...
            if (!auditId) {
                eventsService.emit("SeoToolkit.ViewUpdate", "SiteAuditOverview");
                return;
            }
        ...
        var fn = this.fn = {
            loadCurrentAudit: function (id) {
                $http.get("backoffice/SeoToolkit/SiteAudit/Connect?auditId=" + id + "&clientId=" + fn.getClientId()).then(function (response) {
