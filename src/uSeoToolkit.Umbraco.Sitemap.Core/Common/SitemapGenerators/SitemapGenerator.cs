...
namespace uSeoToolkit.Umbraco.Sitemap.Core.Common.SitemapGenerators
{
    public class SitemapGenerator : ISitemapGenerator
    {
        ...
        public SitemapGenerator(IUmbracoContextFactory umbracoContextFactory, ISettingsService<SitemapConfig> settingsService)
        {
        ...
        private IEnumerable<XElement> GetSelfAndChildren(IPublishedContent content, string culture)
        {
            ...
            if (content.TemplateId > 0)
            {
                ...
                if (!string.IsNullOrWhiteSpace(_settings.ChangeFrequencyFieldAlias) && HasValue(content, _settings.ChangeFrequencyFieldAlias, culture))
                {
                    selfItem.Add(new XElement(_namespace + "changefreq", Value<string>(content, _settings.ChangeFrequencyFieldAlias, culture)));
                }

                if (!string.IsNullOrWhiteSpace(_settings.PriorityFieldAlias) && HasValue(content, _settings.PriorityFieldAlias, culture))
                {
