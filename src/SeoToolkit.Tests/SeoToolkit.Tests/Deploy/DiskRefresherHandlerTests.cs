...
namespace SeoToolkit.Tests.Deploy
{
    ...
    public class DiskRefresherHandlerTests
    {
        ...
        public async Task SeoSettingSaved_WritesArtifactViaConnector()
        {
            ...
            var handler = new SeoSettingDiskRefresherHandler(diskService.Object, factory.Object, signatureService.Object);
            ...
            // The signature cache must be refreshed alongside the disk write, or the entity reads as
            // changed on the next deploy (matches the framework HandleSignatures behaviour).
            signatureService.Verify(s => s.SetSignature(artifact), Times.Once);
        }
        ...
        public async Task ScriptDeleted_DeletesArtifactWithMatchingUdi()
        {
            ...
            var handler = new ScriptDiskRefresherHandler(diskService.Object, factory.Object, signatureService.Object);
            ...
            // Deleting the .uda must also clear its cached signature.
            signatureService.Verify(s => s.ClearSignature(
                It.Is<Udi>(u => u.EntityType == SeoToolkitDeployConstants.UdiEntityType.Script
                    && ((GuidUdi)u).Guid == scriptKey)), Times.Once);
        }
    }
}
